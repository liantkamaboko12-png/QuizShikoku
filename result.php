...
<head>
  ...
  <title>結果発表 - <?= htmlspecialchars($quiz['title']) ?></title>
  ...
  <main class="main-container">
    <section class="metro-section">
        ...
        <?php foreach ($results as $idx => $res): ?>
          <div class="metro-card <?= $res['is_correct'] ? 'correct-border' : 'wrong-border' ?>">
            ...
            <div class="answer-comparison">
              <p>あなたの回答: <strong><?= htmlspecialchars($res['user_answer']) ?></strong></p>
              <?php if (!$res['is_correct']): ?>
                <p class="correct-text">正しい解答: <strong><?= htmlspecialchars($res['correct_answer']) ?></strong></p>
              <?php endif; ?>
            </div>
          </div>
        <?php endforeach; ?>
      ...
      <div class="form-actions">
        <a href="quiz.php?id=<?= htmlspecialchars($id) ?>" class="metro-btn secondary-btn">もう一度挑戦する</a>

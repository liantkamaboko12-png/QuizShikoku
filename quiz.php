...
<head>
  ...
  <title><?= htmlspecialchars($quiz['title']) ?> - 社会科問題集</title>
          ...
          <div class="metro-card">
            ...
            <?php if (!empty($q['image'])): ?>
              <div class="question-image" style="margin-bottom: 16px;">
                <img src="images/<?= htmlspecialchars($q['image']) ?>" alt="問題画像" style="max-width: 100%; height: auto; border: 1px solid #333;">
              </div>
            <?php endif; ?>
              ...
              <div class="multi-input-group">
                <?php foreach ($q['inputs'] as $input): ?>
                  <div class="input-row">
                    ...
                    <input type="text" name="answers[<?= $idx ?>][<?= htmlspecialchars($input['key']) ?>]" class="metro-input" placeholder="回答を入力" required>
                  </div>
                <?php endforeach; ?>
              </div>
